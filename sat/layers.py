...
class Attention(gnn.MessagePassing):
    ...
    def __init__(self, embed_dim, num_heads=8, dropout=0., bias=False,
        ...
        if symmetric:
            ...
            self.to_qk = nn.Linear(embed_dim, embed_dim * 2, bias=bias)   #q、k（128,216）
        self.to_v = nn.Linear(embed_dim, embed_dim, bias=bias)   #128
    ...
    def forward(self,
        ...
        if self.se == 'khopgnn': # k-subgraph SAT
            ...
            x_struct = self.structure_extractor(x, edge_index, edge_attr)   #是不是因为edge_index和edge_attr这两个属性是数据集中
...
class TransformerEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=512, dropout=0.1,
        ...
        if batch_norm:
            ...
            #self.norm3 = nn.BatchNorm1d(1546)
    ...
    def forward(self, x, edge_index, complete_edge_index,
        ...
        if self.pre_norm:
            ...
            x = self.norm1(x)
        #最后输出的两个线性层，x
        x2 = self.linear2(self.dropout(self.activation(self.linear1(x))))  #这个X2是个啥

        #x = self.fc(x)
        x = x + self.dropout2(x2)  #x + self.dropout2(x2)
        if not self.pre_norm:
            x = self.norm2(x)   #x = self.norm2(x)
       #x(1546,1546) x2(1546,1546)
        return x
